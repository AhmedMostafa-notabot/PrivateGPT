...
with st.form('my_form'):
  ...
  if len(uploaded_file_pdf)!=0:
    ...
    vectordb = Chroma.from_texts(text, embedding=embeddings, 
                                     persist_directory=".")
    vectordb.persist()
    memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True,max_tokens_limit=1000)
    pdf_qa = ConversationalRetrievalChain.from_llm(OpenAI(temperature=0.7,model='gpt-3.5-turbo-16k') , vectordb.as_retriever(),memory=memory,max_tokens_limit=3000)
